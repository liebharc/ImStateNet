...
namespace ImStateNet.Mutable
{
    ...
    public class StateMut
    {
        ...
        private readonly object _commitSuspenderLock = new object();
        private readonly ChainTasksQueue _commitDispatcher = new ChainTasksQueue();
...
#pragma warning restore CS8601 // Possible null reference assignment.
        ...
        {
            if (token.IsCancellationRequested && allowCancellation)
            {
                return _state;
            }

            var (stateUpdate, changes) = await _state.Commit(allowCancellation ? token : null);
            lock (_stateLock)
            {
                if (token.IsCancellationRequested && allowCancellation)
                {
                ...
                // Check if any nodes have been added or removed as we were busy calculating the update
                // We can reference compare the nodes as it's an immutable list
                bool haveNodesRemainedTheSame = _state.Nodes == stateUpdate.Nodes;
                if (!haveNodesRemainedTheSame)
                {
                    stateUpdate = stateUpdate.ChangeConfigurationTo(_state);
                }
        ...
        private sealed class DisableAutoCommitScope : IAsyncDisposable, IDisposable
        {
            private readonly StateMut _state;

            public DisableAutoCommitScope(StateMut state)
            {
                lock (state._commitSuspenderLock)
                {
            ...
            {
                lock (_state._commitSuspenderLock)
                {
                    _state._numberOfAutoCommitSuspenders--;
                    if (_state._numberOfAutoCommitSuspenders != 0)
                    {
