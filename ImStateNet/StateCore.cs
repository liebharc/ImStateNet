namespace ImStateNet
{
    ...
    public class State
    {
        ...
        public State ChangeValue<T>(InputNode<T> node, T newValue)
        {
            ...
            return new State(Nodes, values, changes, _initialValues, _versionId);
        }
        ...
        public (State, ImmutableHashSet<INode>) Commit()
        {
            ...
            foreach (var node in Nodes)
            {
                ...
                {
                    var anyDepsChanged = !_changes.Intersect(derivedNode.Dependencies).IsEmpty;
                    if (anyDepsChanged)
                    {
                        var newValue = derivedNode.Calculate(derivedNode.Dependencies.Select(dep => values[dep]).ToList());
                        var oldValue = _initialValues.TryGetValue(node, out var old) ? old : null;
                        values = values.SetItem(node, newValue);

                        if (!derivedNode.AreValuesEqual(oldValue, newValue))
                            changes = changes.Add(node);
                    }
                }
            }

            return (new State(Nodes, values, ImmutableHashSet<INode>.Empty, values), changes);
        }
